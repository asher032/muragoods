  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (!mouseInside) {
        ...
        gridRef.current.querySelectorAll('.magic-bento-card').forEach((card: HTMLElement) => card.style.setProperty('--glow-intensity', '0'));
        return;
      }
    ...
    <>
      ...
      <div className="card-grid bento-section" ref={gridRef}>
        {cardData.map((card, index) => {
          ...
          const cardProps = { className: baseClassName, style: { backgroundColor: card.color, '--glow-color': glowColor } as any };
          if (enableStars) {
            return (
