...
const MaskedHeading: React.FC<Props> = ({
  ...
  const baseRefs = useRef<HTMLSpanElement[]>([]);
  const glyphRefs = useRef<SVGTextElement[]>([]);
  ...
  useEffect(() => {
    ...
    const play = () => {
      ...
      if (reveal === 'rise') {
        ...
        gsap.set(glyphs, { y: 0 });
        const state = { p: 100 };
        tweenRef.current = gsap.to(state, { p: 0, duration, ease: 'power3.inOut', overwrite: 'auto', onUpdate: () => { layer.style.clipPath = `inset(0% ${state.p}% 0% 0%)`; } });
      } else {
        gsap.set(glyphs, { y: 0 });
  ...
  return (
    <Tag ref={rootRef} className={`masked-heading ${className}`.trim()} style={{ textAlign: align, fontWeight: weight, letterSpacing: `${tracking}em`, lineHeight, ...style }} {...rest}>
