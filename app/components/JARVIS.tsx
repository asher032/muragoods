...
type Panel = 'chat' | 'memory' | 'history' | 'privacy' | 'code' | 'manage';
const ADMIN_EMAILS = ['muragoods0@gmail.com', 'mhaxthedog@gmail.com'];

// ─── Component ───────────────────────────────────────────────
export function JARVIS({ open, onClose, isFullPage }: { open: boolean; onClose: () => void; isFullPage?: boolean }) {
  ...
  useEffect(() => { if (userEmail) setIsAdmin(ADMIN_EMAILS.includes(userEmail)); }, [userEmail]);
  ...
  const onSpeechEnd = useCallback(() => { setSpeaking(false); setCurrentAudio(null); }, []);
  ...
  const processMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      const jarvisMsg: Message = {
        ...
        role: 'jarvis',
        text: resp || 'I had trouble processing that. Can you try again?',
      ...
      if (action === 'navigate' && actionParams?.path) setTimeout(() => router.push(actionParams.path), 500);
      ...
      setMessages(prev => [...prev, { id: (Date.now() + 1).toString(), role: 'jarvis', text: 'Connection issue. Please try again.', timestamp: new Date() }]);
    }
    setProcessing(false);
  ...
  const handleRecordingStop = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success && data.text) handleSendRef.current(data.text);
    } catch { /* empty */ }
    setProcessing(false);
  }, []);
  ...
  const capturePhoto = useCallback(() => {
    ...
    if (ctx) { ctx.drawImage(video, 0, 0); setCameraStream(null); cameraStream?.getTracks().forEach(t => t.stop()); processMessage('Analyze this image'); }
  }, [cameraStream, processMessage]);

  // ─── Code editing ───────────────────────────────────────
  const loadCodeFiles = useCallback(async (path?: string) => {
    try { const res = await fetch(`/api/jarvis/code?action=list&email=${userEmail}${path ? `&path=${path}` : ''}`); const data = await res.json(); if (data.success) setCodeFiles(data.data); } catch { /* empty */ }
  }, [userEmail]);
  const loadFileContent = useCallback(async (filePath: string) => {
    try { const res = await fetch(`/api/jarvis/code?action=read&email=${userEmail}&path=${encodeURIComponent(filePath)}`); const data = await res.json(); if (data.success) { setFileContent(data.data.content); setSelectedFile(filePath); } } catch { /* empty */ }
  }, [userEmail]);
  const applyCodeEdit = useCallback(async () => {
    if (!selectedFile || !editInstruction) return;
    setEditLoading(true);
    try {
      const res = await fetch('/api/jarvis/code', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'edit', email: userEmail, path: selectedFile, instruction: editInstruction }) });
      const data = await res.json();
      if (data.success) { setFileContent(data.data.content); setEditInstruction(''); setMessages(prev => [...prev, { id: Date.now().toString(), role: 'jarvis', text: `Updated ${selectedFile} successfully.`, timestamp: new Date() }]); }
      else { setMessages(prev => [...prev, { id: Date.now().toString(), role: 'jarvis', text: `Edit failed: ${data.error}`, timestamp: new Date() }]); }
    } catch { /* empty */ }
  ...
  const loadSiteStats = useCallback(async () => {
    try { const res = await fetch('/api/jarvis/manage', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'site-stats', email: userEmail }) }); const data = await res.json(); if (data.success) setSiteStats(data.data); } catch { /* empty */ }
  }, [userEmail]);
  const loadUsers = useCallback(async () => {
    try { const res = await fetch('/api/jarvis/manage', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'list-users', email: userEmail }) }); const data = await res.json(); if (data.success) setUserList(data.data.users); } catch { /* empty */ }
  }, [userEmail]);
  const loadMemories = useCallback(async () => {
    try { const res = await fetch(`/api/jarvis/memory?userId=${userId || 'anonymous'}`); const data = await res.json(); if (data.success) setMemories(data.memories || []); } catch { /* empty */ }
  }, [userId]);
  ...
  const resetChat = useCallback(() => {
    setMessages([]);
    setCommandHistory([]);
    setMessages([{ id: 'w2', role: 'jarvis', text: 'Chat cleared. What can I help you with?', intent: 'INFORMATION', timestamp: new Date() }]);
  }, []);
  ...
  useEffect(() => {
    ...
    if (panel === 'code' && isAdmin) loadCodeFiles();
    if (panel === 'manage' && isAdmin) { loadSiteStats(); loadUsers(); }
  }, [open, panel, isAdmin]);

  // ─── Keyboard shortcut (only for panel mode) ────────────
  useEffect(() => {
  ...
  // Panel styling for full page vs side panel
  const panelStyle = isFullPage ? {
    position: 'fixed' as const, top: 0, left: 0, width: '100%', height: '100%', maxWidth: '100%', maxHeight: '100vh', zIndex: 9999, display: 'flex' as const, flexDirection: 'column' as const, background: 'linear-gradient(180deg, #0a0e1a 0%, #0d1220 50%, #111827 100%)', boxShadow: 'none', fontFamily: 'system-ui, -apple-system, sans-serif', color: '#e0e0e0', overflow: 'hidden' as const,
  } : {
    position: 'fixed' as const, bottom: 0, right: 0, width: '100%', height: '100%', maxWidth: '480px', maxHeight: '100vh', zIndex: 9999, display: 'flex' as const, flexDirection: 'column' as const, background: 'linear-gradient(180deg, #0a0e1a 0%, #0d1220 50%, #111827 100%)', borderLeft: '1px solid rgba(0,229,255,0.15)', boxShadow: '-10px 0 40px rgba(0,0,0,0.5)', fontFamily: 'system-ui, -apple-system, sans-serif', color: '#e0e0e0', overflow: 'hidden' as const,
  };

  return (
    <div className="jarvis-overlay" onClick={(e) => { if (e.target === e.currentTarget && !isFullPage) onClose(); }} style={isFullPage ? { background: '#0a0e1a' } : {}}>
      <div className="jarvis-panel" style={panelStyle}>
        {/* ─── Top Bar ────────────────────────────────── */}
        ...
        {cameraStream && (
          <div style={{ position: 'relative', margin: '0 16px 10px', borderRadius: '12px', overflow: 'hidden', border: '1px solid rgba(0,255,136,0.3)' }}>
            ...
            <div style={{ position: 'absolute', bottom: '10px', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '8px' }}>
              ...
              <button onClick={() => { cameraStream?.getTracks().forEach(t => t.stop()); setCameraStream(null); }} style={{ padding: '8px 16px', borderRadius: '8px', background: 'rgba(255,68,68,0.2)', border: '1px solid #ff4444', color: '#ff4444', cursor: 'pointer' }}>✕ Close</button>
            </div>
          </div>
        )}
            ...
            {processing && (
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                ...
                <div style={{ padding: '10px 14px', borderRadius: '12px 12px 12px 2px', background: 'rgba(0,229,255,0.06)', border: '1px solid rgba(0,229,255,0.1)' }}>
                  <span style={{ fontSize: '12px', color: '#00e5ff', fontFamily: 'monospace' }}>Thinking...</span>
                </div>
              </div>
            )}
        ...
        {panel === 'memory' && (
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
            ...
            {memories.length === 0 ? <p style={{ fontSize: '11px', color: '#666' }}>No memories saved yet.</p> : memories.map(m => (
        ...
        {panel === 'code' && isAdmin && (
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginBottom: '12px' }}>
              {codeFiles.map(f => (
                <button key={f.path} onClick={() => loadFileContent(f.path)} style={{ padding: '4px 8px', borderRadius: '4px', background: selectedFile === f.path ? 'rgba(0,255,136,0.2)' : 'rgba(255,255,255,0.05)', border: `1px solid ${selectedFile === f.path ? '#00ff88' : 'rgba(255,255,255,0.1)'}`, color: selectedFile === f.path ? '#00ff88' : '#aaa', fontSize: '9px', cursor: 'pointer', fontFamily: 'monospace' }}>{f.name}</button>
              ))}
            </div>
            {selectedFile && (
              <div>
                ...
                <textarea value={editInstruction} onChange={e => setEditInstruction(e.target.value)} placeholder="Tell me what to change..." style={{ width: '100%', marginTop: '8px', padding: '8px', borderRadius: '8px', background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(0,255,136,0.2)', color: '#ccc', fontSize: '11px', resize: 'vertical', minHeight: '60px' }} />
                <button onClick={applyCodeEdit} disabled={editLoading || !editInstruction} style={{ marginTop: '8px', padding: '6px 16px', borderRadius: '6px', background: editLoading ? 'rgba(0,255,136,0.05)' : 'rgba(0,255,136,0.15)', border: '1px solid #00ff88', color: '#00ff88', fontSize: '10px', cursor: editLoading ? 'not-allowed' : 'pointer' }}>{editLoading ? '⏳ Applying...' : '✨ Apply Edit'}</button>
              </div>
            )}
          </div>
        )}
        ...
        {panel === 'manage' && isAdmin && (
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
            ...
            {siteStats && <div style={{ padding: '12px', borderRadius: '8px', background: 'rgba(255,170,0,0.05)', border: '1px solid rgba(255,170,0,0.1)', marginBottom: '12px' }}><pre style={{ fontSize: '10px', color: '#ccc', fontFamily: 'monospace', whiteSpace: 'pre-wrap' }}>{JSON.stringify(siteStats, null, 2)}</pre></div>}
            ...
            {userList.map(u => (
              <div key={u.userId} style={{ padding: '8px 12px', borderRadius: '8px', background: 'rgba(255,170,0,0.05)', border: '1px solid rgba(255,170,0,0.1)', marginBottom: '6px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div><p style={{ fontSize: '11px', color: '#ccc' }}>{u.name || u.email}</p><p style={{ fontSize: '9px', color: '#666' }}>{u.email}</p></div>
        ...
        {panel === 'chat' && (
          <div style={{ padding: '12px 16px', borderTop: '1px solid rgba(255,255,255,0.05)', background: 'rgba(0,0,0,0.3)', flexShrink: 0 }}>
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              ...
              <button onClick={() => handleSend()} disabled={!input.trim() || processing} style={{ padding: '10px 14px', borderRadius: '10px', background: input.trim() ? 'rgba(0,229,255,0.15)' : 'rgba(255,255,255,0.05)', border: '1px solid rgba(0,229,255,0.2)', color: input.trim() ? '#00e5ff' : '#555', cursor: input.trim() ? 'pointer' : 'not-allowed', fontSize: '13px' }}>➤</button>
            </div>
            <div style={{ display: 'flex', gap: '6px', marginTop: '8px', justifyContent: 'center', flexWrap: 'wrap' }}>
              {['Open menu', 'My orders', 'Play games', 'Check points'].map(cmd => (
                <button key={cmd} onClick={() => handleSend(cmd)} style={{ padding: '4px 8px', borderRadius: '6px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', color: '#888', fontSize: '9px', cursor: 'pointer' }}>{cmd}</button>
              ))}
            </div>
          </div>
        )}
