...
import { Bird, Cloud, Eye, Flower2, Handshake, Heart, HeartCrack, Mail, MailOpen } from 'lucide-react';
...
const catColors: Record<string, { emoji: React.ReactNode; color: string }> = {
  ...
  Letter: { emoji: <MailOpen className="inline-block" style={{ verticalAlign: '-0.15em', flexShrink: 0 }} aria-hidden />, color: '#c896ff' },
};
...
export default function ViewLetter() {
  ...
  useEffect(() => {
    async function fetchLetter() {
      try {
        const res = await fetch(`/api/untold-words/anonymous?id=${shortId}`);
        const result = await res.json();
        if (result.success) {
          setLetter(result.data);
          setLikes(result.data.likes || 0);
        } else {
          setError('Letter not found or has been removed.');
        }
  ...
  const handleReport = async () => {
    if (reported) return;
    try {
      await fetch(`/api/untold-words/anonymous?id=${shortId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const cat = catColors[letter.category] || catColors['Letter'];
    ...
    return (
      <main style={{ minHeight: '100vh', background: '#0a0a18', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        ...
        <div style={{ textAlign: 'center', position: 'relative', zIndex: 1 }}>
          ...
          <span style={{ fontFamily: 'var(--font-arcade)', fontSize: '9px', color: cat.color, background: cat.color + '15', padding: '4px 12px', borderRadius: '8px', marginBottom: '16px', display: 'inline-block' }}>{letter.category}</span>
          <h1 style={{ fontFamily: 'var(--font-arcade)', fontSize: 'clamp(18px, 5vw, 26px)', color: cat.color, marginTop: '16px', marginBottom: '24px' }}>{letter.title}</h1>
      ...
      <div style={{ maxWidth: '520px', margin: '0 auto', opacity: showContent ? 1 : 0, transform: showContent ? 'translateY(0)' : 'translateY(30px)', transition: 'all 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)' }}>
        <div style={{ background: '#141428', border: `1px solid ${cat.color}30`, borderRadius: '24px', overflow: 'hidden', boxShadow: `0 16px 48px rgba(0,0,0,0.3), 0 0 60px ${cat.color}08` }}>
          <div style={{ padding: '36px 32px 24px', textAlign: 'center', borderBottom: `1px solid ${cat.color}15` }}>
            ...
            <span style={{ fontFamily: 'var(--font-arcade)', fontSize: '8px', color: cat.color, background: cat.color + '15', padding: '3px 10px', borderRadius: '6px' }}>{letter.category}</span>
            <h1 style={{ fontFamily: 'Georgia, serif', fontSize: '20px', color: cat.color, marginTop: '16px', marginBottom: '4px' }}>{letter.title}</h1>
            <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.2)' }}>{new Date(letter.createdAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
          </div>
