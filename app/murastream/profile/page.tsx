...
import {
  ...
  AlertIcon, GearIcon, PackageIcon, ScrollIcon, SparklesIcon,
...
type OrderStats = { count: number; spent: number; points: number };

function StatCard({ label, value, icon }: { label: string; value: string | number; icon: React.ReactNode }) {
  return (
    <div style={{
      background: 'var(--ms-surface)', borderRadius: '12px', border: '1px solid var(--ms-border)',
      padding: '18px 16px', flex: '1 1 130px', minWidth: '130px',
    }}>
      <div style={{ color: '#E50914', marginBottom: '10px' }}>{icon}</div>
      <div style={{ fontFamily: 'var(--font-arcade)', fontSize: '22px', color: 'var(--ms-text)', marginBottom: '4px' }}>
        {value}
      </div>
      <div style={{ fontFamily: 'var(--font-arcade)', fontSize: '8px', color: 'var(--ms-text-faint)', letterSpacing: '0.1em' }}>
        {label}
      </div>
    </div>
  );
...
function Row({ icon, label, value, href, onClick, danger }: {
  ...
  const inner = (
    <>
      <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <span style={{ fontFamily: '"Lucida Sans", sans-serif', fontSize: '13px', color: danger ? '#e63946' : 'var(--ms-text)' }}>{label}</span>
      </span>
      {value !== undefined
        ? <span style={{ fontFamily: 'var(--font-arcade)', fontSize: '10px', color: '#E50914' }}>{value}</span>
        : <ChevronRightIcon size={14} color="var(--ms-text-ghost)" />}
    </>
  );
  const base = {
    width: '100%', padding: '14px 16px', borderBottom: '1px solid var(--ms-border)',
  ...
  if (href) return <Link href={href} style={{ ...base, borderBottom: '1px solid var(--ms-border)' }}>{inner}</Link>;
...
export default function MuraStreamProfilePage() {
  ...
  const [orderStats, setOrderStats] = useState<OrderStats>({ count: 0, spent: 0, points: 0 });
  ...
  // Full list feeds the stats; the 3 most recent render below.
  useEffect(() => {
    try {
      const raw = localStorage.getItem('user');
      ...
      fetch(`/api/orders?userId=${encodeURIComponent(email)}`)
        ...
        .then(d => {
          const all: MiniOrder[] = d.data || [];
          setOrders(all.slice(0, 3));
          setOrderStats({
            count: all.length,
            spent: all.reduce((s, o) => s + (Number(o.total) || 0), 0),
            points: all.reduce((s, o) => s + (Number(o.pointsEarned) || 0), 0),
          });
        })
        .catch(() => { /* empty */ });
    } catch { /* empty */ }
    ...
    <div className="ms-page-pad">
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ fontFamily: 'var(--font-arcade)', fontSize: '14px', color: 'var(--ms-text)', margin: 0 }}>
            {user?.name || 'Not signed in'}
          </p>
          <p style={{ fontFamily: '"Lucida Sans", sans-serif', fontSize: '12px', color: 'var(--ms-text-faint)', margin: '4px 0 0', wordBreak: 'break-all' }}>
        ...
        <Link href="/account/profile" title="Edit your MuraGoods account"
          style={{ color: 'var(--ms-text-dim)', display: 'flex', padding: 8, borderRadius: 8, border: '1px solid rgba(255,255,255,0.08)' }}>
          <GearIcon size={15} />
        </Link>
      </div>
      ...
      {!user && (
        <div style={{
          ...
          borderRadius: 10, padding: '12px 16px', marginBottom: 24, fontSize: 13, color: 'var(--ms-text-muted)',
        }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        <p style={{ fontFamily: 'var(--font-arcade)', fontSize: '9px', color: 'var(--ms-text-ghost)', letterSpacing: '0.15em', margin: '0 0 12px' }}>
          WATCH STATS
        </p>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <div style={{ background: 'var(--ms-surface)', borderRadius: '12px', border: '1px solid var(--ms-border)', overflow: 'hidden' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        {orderStats.count > 0 && (
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '12px' }}>
            <StatCard label="ORDERS" value={orderStats.count} icon={<PackageIcon size={20} />} />
            <StatCard label="TOTAL SPENT" value={`₱${orderStats.spent}`} icon={<FilmIcon size={20} />} />
            <StatCard label="POINTS EARNED" value={orderStats.points} icon={<SparklesIcon size={20} />} />
          </div>
        )}
        <div style={{ background: 'var(--ms-surface)', borderRadius: '12px', border: '1px solid var(--ms-border)', overflow: 'hidden' }}>
          {orders.length === 0 ? (
            <div style={{ padding: '14px 16px', fontSize: 13, color: 'var(--ms-text-faint)' }}>
            ...
            <>
              {orders.map((o, i) => (
                <Link key={o._id} href={`/order/${o._id}`} style={{
                  padding: '14px 16px', borderTop: i === 0 ? 'none' : '1px solid var(--ms-border)',
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                    ...
                    <span style={{ fontFamily: '"Lucida Sans", sans-serif', fontSize: 13, color: 'var(--ms-text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
