...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    items: [
      { label: 'FLAG', text: 'Report button in the player — flag broken or ad-heavy sources. Reports feed trust scores and the admin dashboard.' },
      { label: 'TRUST', text: 'Trust badges on the active source, backed by real community reports (Trusted / Mixed / Low / Reported broken).' },
      { label: 'CACHED', text: 'Per-title source memory — repeat plays skip the health probe entirely and open your last working provider instantly.' },
      { label: 'S · N · F', text: 'Keyboard shortcuts: S cycles sources, N jumps to the next episode, F fullscreen, M mute, ←/→ seek episodes.' },
      { label: 'AUTO', text: 'Instant next-episode autoplay when the player signals the episode ended — no countdown.' },
    ],
  },
  {
    id: 'dramas',
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'HLS', text: 'Native HLS player with a true quality selector (Auto/1080p/720p/…), progress resume, and no third-party embeds.' },
      { label: 'NO ADS', text: 'The only section where zero ads is guaranteed by construction — there is no external player to inject anything.' },
    ],
  },
  {
    id: 'player-v2',
    ...
    items: [
      { label: 'PROBE', text: 'Server-side source verification — players probe all providers for the exact title before opening, so dead sources are skipped instead of silently stalling.' },
      { label: 'CLEAN', text: 'Ad-heavy providers demoted to last-resort; notorious popup sources removed entirely.' },
      ...
      { label: 'FALLBACK', text: 'Automatic fallback chain: if a source fails mid-playback it is marked red and playback hops to the next healthy provider.' },
    ],
  },
];
